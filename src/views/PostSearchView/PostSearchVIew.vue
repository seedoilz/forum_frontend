  ...
  <div>
    ...
    <el-container>
      ...
      <el-main style="display: flex;align-items: center;flex-direction: column;margin-top: 10%">
        <h1 style="left: 20%;position: absolute;font-size: 3rem;margin-top: 10px;margin-bottom: 10px;color: #26bbff">//{{this.$route.params.word}}</h1>
        <!--        main-->
        <div
...
import {postSearch} from '../../network/any'
...
export default {
  ...
  watch: {
    $route (to, from) {
      ...
      if (to.path.startsWith('/post_search')) {
        // 执行重新渲染的逻辑，例如重新加载数据
        this.loadData()
      }
    },
  ...
  methods: {
    ...
    loadData () {
      ...
      postSearch(this.options, config).then((res) => {
        console.log(res)
        if (res.code === 200) {
